...
function Sign() {
  ...
  const submit = async (data) => {
    try {
      const res = await axios.post("http://127.0.0.1:7056/offline", {
        username: data.name,
        email: data.email,
    ...
    <div className='bg-amber-300 items-center h-screen w-full bg-[url("/images.jpeg")] bg-cover bg-center'>
      ...
      <div className=' items-center mx-auto h-140 w-120 mt-10  rounded-2xl border border-gray-200 p-6 '>
        <form onSubmit={handleSubmit(submit)} className='grid  '>
          ...
          <button type="button" className='text-xl bg-[url("/google.png")] bg-contain bg-no-repeat bg-center w-full h-8 mt-9 mx-auto items-center border border-amber-50  rounded-xl text-white'>
